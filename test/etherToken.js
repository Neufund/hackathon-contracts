...
contract("EtherToken", async ([_, reclaimer, investor1, investor2]) => {
  ...
  it("should reclaim ether above etherToken supply", async () => {
    ...
    await chain.etherToken.deposit(investor2, deposit, {
      from: investor2,
      value: deposit
    });
    ...
    expect(reclaimerEthAfterBalance).to.be.bignumber.eq(
      reclaimerEthBalance.add(amount).sub(gasCost)
    );
    // deposit stays
    expect(
      await web3.eth.getBalance(chain.etherToken.address)
    ).to.be.bignumber.eq(deposit);
    // nothing changed by further reclaims
    await chain.etherToken.reclaim(reclaimEther, {
      from: reclaimer,
      gasPrice
    });
    expect(
  ...
  it("should be able to reclaim etherToken balance on itself", async () => {
    ...
    await chain.etherToken.transfer(chain.etherToken.address, toReclaim, {
      from: investor2
    });
    await allowToReclaim(reclaimer);
    // reclaim from itself
    await chain.etherToken.reclaim(chain.etherToken.address, {
      from: reclaimer
    });
    // only toReclaim is reclaimed
    expect(await chain.etherToken.balanceOf(reclaimer)).to.be.bignumber.eq(
      toReclaim
    );
  });
});
